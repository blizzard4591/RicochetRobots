...
void to_json(json& j, MapBuilder::JsonBarrier const& o) {
	j = json{ { "type", static_cast<int>(o.barrierType) },{ "color", static_cast<int>(o.barrierColor) },{ "x", o.position.x },{ "y", o.position.y } };
}
...
void MapBuilder::addGoal(JsonGoal const& jsonGoal) {
	if (jsonGoal.goalColor <= Color::INVALID_MIN || jsonGoal.goalColor >= Color::INVALID_MAX) {
...
void MapBuilder::addBarrier(JsonBarrier const& jsonBarrier) {
	if (jsonBarrier.barrierType <= BarrierType::INVALID_MIN || jsonBarrier.barrierType >= BarrierType::INVALID_MAX) {
		throw std::exception("BarrierType value out of range!");
	} else if (jsonBarrier.barrierColor <= Color::INVALID_MIN || jsonBarrier.barrierColor >= Color::INVALID_MAX) {
		throw std::exception("Color value out of range!");
	}
